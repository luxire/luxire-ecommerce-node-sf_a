var luxire = angular.module('luxire')
  ...
  luxire.controller('productsHomeController', function ($scope, products, allTaxons, luxireProperties, luxireVendor, fileReader, fileUpload, prototypeObject, $uibModal, $log, createProductModalService, $state, csvFileUpload, ImageHandler) {
    $scope.loading = true;
    ...
    $scope.deleteProducts = function (product, index) {
      ...
      modalInstance.result.then(function (data) {
        $scope.loading = true;
        products.deleteProduct(data.product.id).then(function (data) {
          $scope.loading = false;
          $scope.masterSearchJson.splice(index, 1);
...
// deleteProductController is used by product delete modal
luxire.controller('deleteProductController', function ($scope, $uibModalInstance, product) {
  $scope.product = product;
  $scope.ok = function () {
    $uibModalInstance.close({ key: "closed successfully", product: product });
  };
